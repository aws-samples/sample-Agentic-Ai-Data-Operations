"""Supplier Chain Data Pipeline DAG (manual trigger).

Includes a PII tagging step (Lake Formation LF-Tags on contact_email,
supplier_address) after Silver quality passes.
"""
...
def _apply_pii_tags(**context):
    """Apply Lake Formation LF-Tags to PII columns.

    Tool routing decision:
      - Intent: "tag PII columns" -> TOOL_ROUTING.md Step 5: lake-formation-grant
      - MCP server: lakeformation (REQUIRED)
      - Tags: PII_Classification=CONTAINS_PII, PII_Type=[EMAIL,ADDRESS], Data_Sensitivity=HIGH
      - Targets: contact_email (EMAIL), supplier_address (ADDRESS)
    """
    print(
        "PII tagging: contact_email -> PII_Type=EMAIL, supplier_address -> "
        "PII_Type=ADDRESS, both Data_Sensitivity=HIGH on "
        f"{WORKLOAD}_db.silver_{WORKLOAD}"
    )


with DAG(
    dag_id=f"{WORKLOAD}_pipeline",
    description="Manual ETL: Bronze -> Silver for buyer-supplier relationships (PII)",
    schedule=None,
    ...
    tags=[WORKLOAD, "supply-chain", "manual", "pii"],
    doc_md=__doc__,
) as dag:
    ...
    wait_for_source = S3KeySensor(
        ...
        bucket_key=f"raw/supplier_chain_data/*.csv",
        poke_interval=300,
        timeout=3600,
        mode="poke",
    )
    ...
    with TaskGroup("bronze") as bronze_group:
        ingest = GlueJobOperator(
            ...
            script_args={
                ...
                "--source_prefix": "raw/supplier_chain_data/",
    ...
    pii_tagging = PythonOperator(
        task_id="pii_tagging",
        python_callable=_apply_pii_tags,
    )
    ...
    wait_for_source >> bronze_group >> silver_group >> pii_tagging >> alert_on_failure
