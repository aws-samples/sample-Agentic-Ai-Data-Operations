...
class TestSourceConfig:
    def test_dedup_key(self):
        ...
        assert config["ingestion"]["dedup_key"] == ["entity_id", "price_date"]
...
class TestTransformationConfig:
    def test_dedup_keep_latest(self):
        ...
        assert dedup["key"] == ["entity_id", "price_date"]
...
class TestQualityConfig:
    ...
    def test_pk_uniqueness_critical(self):
        ...
        assert u[0]["columns"] == ["entity_id", "price_date"]
        assert u[0]["severity"] == "critical"

    def test_close_price_positive_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        rule = next(r for r in validity if r.get("column") == "close_price")
        assert rule["severity"] == "critical"
        assert "> 0" in rule["rule"]

    def test_volume_nonneg_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        rule = next(r for r in validity if r.get("column") == "volume")
        assert rule["severity"] == "critical"

    def test_market_cap_positive_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        rule = next(r for r in validity if r.get("column") == "market_cap")
        assert rule["severity"] == "critical"
...
class TestScripts:
    def test_silver_dedups_on_pk(self):
        ...
        assert 'partitionBy("entity_id", "price_date")' in s
    ...
    def test_quality_enforces_critical_numerics(self):
        ...
        assert "close_price" in s and "volume" in s and "market_cap" in s
...
class TestSQL:
    ...
    def test_silver_pk_columns_not_null(self):
        sql = (SQL_DIR / "silver" / "create_silver_table.sql").read_text()
        for col in ("entity_id", "price_date", "close_price", "currency"):
