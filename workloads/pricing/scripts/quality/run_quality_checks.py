"""Quality checks for pricing Silver.
...
Critical:
  * Completeness on entity_id, price_date, close_price, currency
  * Uniqueness of (entity_id, price_date)
  * close_price > 0
  * volume >= 0
  * market_cap > 0
...
checks_total += 1
unique = df.select("entity_id", "price_date").distinct().count()
...
checks_total += 1
bad_close = df.filter(F.col("close_price") <= 0).count()
if bad_close == 0:
    checks_passed += 1
else:
    critical_failures.append(f"close_price <= 0: {bad_close} row(s)")

checks_total += 1
neg_volume = df.filter(F.col("volume") < 0).count()
if neg_volume == 0:
    checks_passed += 1
else:
    critical_failures.append(f"volume < 0: {neg_volume} row(s)")

checks_total += 1
bad_mcap = df.filter(F.col("market_cap") <= 0).count()
if bad_mcap == 0:
    checks_passed += 1
else:
    critical_failures.append(f"market_cap <= 0: {bad_mcap} row(s)")
