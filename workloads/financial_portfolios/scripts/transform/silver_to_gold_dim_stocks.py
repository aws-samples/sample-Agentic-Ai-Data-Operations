#!/usr/bin/env python3
"""
Silver → Gold transformation for dim_stocks (Dimension Table)
AWS Glue ETL script with local mode fallback

Transformation:
- Copy all columns from silver.stocks to gold.dim_stocks
- SCD Type 1: Overwrite with current state
- No aggregation or joining needed (dimension table)

Usage:
  Glue: --silver_path s3://bucket/silver/stocks/ --gold_path s3://bucket/gold/dim_stocks/
  Local: --local --silver_path ./output/silver/stocks.parquet --gold_path ./output/gold/dim_stocks.parquet
"""
...
def transform_glue_mode(glue_context, args):
    ...
    silver_table = "glue_catalog.financial_portfolios_db.silver_stocks"
    ...
    gold_table = "glue_catalog.financial_portfolios_db.gold_dim_stocks"
    gold_df.write \
        .format("iceberg") \
        .mode("overwrite") \
    ...
    lineage = {
        ...
        "source": "glue_catalog.financial_portfolios_db.silver_stocks",
        "target": "glue_catalog.financial_portfolios_db.gold_dim_stocks",
