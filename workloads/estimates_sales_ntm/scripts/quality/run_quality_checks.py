"""Quality checks for estimates_sales_ntm Silver.
...
Critical:
  * Completeness on entity_id, estimate_date, metric, value_usd, currency
  * Uniqueness of (entity_id, estimate_date, metric)
  * value_usd >= 0
...
checks_total += 1
unique = df.select("entity_id", "estimate_date", "metric").distinct().count()
