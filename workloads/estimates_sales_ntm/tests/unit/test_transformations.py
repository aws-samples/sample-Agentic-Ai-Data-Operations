"""Unit tests for estimates_sales_ntm configs, scripts, DAG, and SQL."""
...
class TestSourceConfig:
    def test_dedup_key(self):
        ...
        assert config["ingestion"]["dedup_key"] == ["entity_id", "estimate_date", "metric"]
...
class TestTransformationConfig:
    def test_dedup_keep_latest(self):
        ...
        assert dedup["key"] == ["entity_id", "estimate_date", "metric"]
...
class TestQualityConfig:
    ...
    def test_pk_uniqueness_critical(self):
        ...
        assert u[0]["columns"] == ["entity_id", "estimate_date", "metric"]
...
class TestScripts:
    ...
    def test_silver_dedups_on_pk(self):
        s = (SCRIPTS_DIR / "transform" / "bronze_to_silver.py").read_text()
        assert 'partitionBy("entity_id", "estimate_date", "metric")' in s
...
class TestDAG:
    DAG_FILE = "estimates_sales_ntm_pipeline.py"
...
class TestSQL:
    ...
    def test_silver_pk_columns_not_null(self):
        sql = (SQL_DIR / "silver" / "create_silver_table.sql").read_text()
        for col in ("entity_id", "estimate_date", "metric", "value_usd", "currency"):
