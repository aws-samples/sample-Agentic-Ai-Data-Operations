"""Quality checks for estimates_ratings Silver.
...
Critical:
  * Completeness on entity_id, rating_date, consensus_rating
  * Uniqueness of (entity_id, rating_date)
  * target_price_low <= mean <= high
  * buy_count + hold_count + sell_count >= 1

Warning:
  * consensus_rating in (Buy, Hold, Sell, Outperform, Underperform)
  * currency ISO 4217
...
checks_total += 1
unique = df.select("entity_id", "rating_date").distinct().count()
...
checks_total += 1
bad_price_range = df.filter(
    (F.col("target_price_low") > F.col("target_price_mean"))
    | (F.col("target_price_mean") > F.col("target_price_high"))
).count()
if bad_price_range == 0:
    checks_passed += 1
else:
    critical_failures.append(
        f"target_price range invariant violated: {bad_price_range} row(s)"
    )

checks_total += 1
total_analysts = (
    F.coalesce(F.col("buy_count"), F.lit(0))
    + F.coalesce(F.col("hold_count"), F.lit(0))
    + F.coalesce(F.col("sell_count"), F.lit(0))
)
no_analysts = df.filter(total_analysts < 1).count()
if no_analysts == 0:
    checks_passed += 1
else:
    critical_failures.append(f"total analyst count < 1: {no_analysts} row(s)")

# --- Warning ----------------------------------------------------------------

allowed = ["Buy", "Hold", "Sell", "Outperform", "Underperform"]
bad_rating = df.filter(~F.col("consensus_rating").isin(allowed)).count()
if bad_rating > 0:
    warnings.append(f"consensus_rating outside vocabulary: {bad_rating} row(s)")
