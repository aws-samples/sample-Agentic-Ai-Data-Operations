"""Estimates Ratings Pipeline DAG (manual trigger)."""
...
with DAG(
    ...
    description="Manual ETL: Bronze -> Silver for sell-side ratings/target prices",
    schedule=None,
    ...
    wait_for_source = S3KeySensor(
        ...
        bucket_key=f"raw/estimates/ratings/*.csv",
        poke_interval=300,
        timeout=3600,
        mode="poke",
    )
    ...
    with TaskGroup("bronze") as bronze_group:
        ingest = GlueJobOperator(
            ...
            script_args={
                ...
                "--source_prefix": "raw/estimates/ratings/",
