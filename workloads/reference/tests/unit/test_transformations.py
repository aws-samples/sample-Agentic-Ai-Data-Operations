...
class TestSourceConfig:
    def test_dedup_key(self):
        ...
        assert config["ingestion"]["dedup_key"] == ["entity_id", "classification_system"]
...
class TestTransformationConfig:
    def test_dedup_keep_latest(self):
        ...
        assert dedup["key"] == ["entity_id", "classification_system"]
...
class TestQualityConfig:
    ...
    def test_pk_uniqueness_critical(self):
        ...
        assert u[0]["columns"] == ["entity_id", "classification_system"]
        assert u[0]["severity"] == "critical"

    def test_code_format_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        for col in ("sector_code", "industry_group_code", "industry_code"):
            rule = next(r for r in validity if r.get("column") == col)
            assert rule["severity"] == "critical"
            assert "[0-9]" in rule["rule"]
...
class TestScripts:
    def test_silver_dedups_on_pk(self):
        ...
        assert 'partitionBy("entity_id", "classification_system")' in s
    ...
    def test_quality_validates_code_format(self):
...
class TestDAG:
    DAG_FILE = "reference_pipeline.py"
...
class TestSQL:
    ...
    def test_silver_pk_columns_not_null(self):
        sql = (SQL_DIR / "silver" / "create_silver_table.sql").read_text()
        for col in ("entity_id", "classification_system",
