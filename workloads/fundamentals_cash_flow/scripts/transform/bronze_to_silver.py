"""Silver transformation: type-cast, validate, latest-wins dedup for cash flow.
...
df = (
    df.withColumn("fiscal_year", F.col("fiscal_year").cast(IntegerType()))
      .withColumn("operating_cash_flow", F.col("operating_cash_flow").cast(DoubleType()))
      .withColumn("capital_expenditure", F.col("capital_expenditure").cast(DoubleType()))
      .withColumn("free_cash_flow", F.col("free_cash_flow").cast(DoubleType()))
      .withColumn("dividends_paid", F.col("dividends_paid").cast(DoubleType()))
      .withColumn("share_buybacks", F.col("share_buybacks").cast(DoubleType()))
      .withColumn("value_usd_fcf", F.col("value_usd_fcf").cast(DoubleType()))
...
if quarantine_count > 0:
    quarantined.write.mode("append").parquet(
        f"s3://{args['data_lake_bucket']}/quarantine/fundamentals_cash_flow/"
    )
