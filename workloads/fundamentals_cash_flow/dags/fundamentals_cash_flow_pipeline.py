"""Fundamentals Cash Flow Pipeline DAG (manual trigger)."""
...
with DAG(
    ...
    description="Manual ETL: Bronze -> Silver for annual cash flow fundamentals",
    schedule=None,
    ...
    wait_for_source = S3KeySensor(
        ...
        bucket_key=f"raw/fundamentals/cash_flow/*.csv",
        poke_interval=300,
        timeout=3600,
        mode="poke",
    )
    ...
    with TaskGroup("bronze") as bronze_group:
        ingest = GlueJobOperator(
            ...
            script_args={
                ...
                "--source_prefix": "raw/fundamentals/cash_flow/",
