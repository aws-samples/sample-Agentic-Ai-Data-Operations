"""Unit tests for fundamentals_cash_flow."""
...
class TestQualityConfig:
    ...
    def test_fcf_identity_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        consistency = config["dimensions"]["consistency"]
        rule = next(
            r for r in consistency
            if "operating_cash_flow" in r.get("rule", "") and "capital_expenditure" in r.get("rule", "")
        )
...
class TestScripts:
    ...
    def test_quality_enforces_fcf_identity(self):
        s = (SCRIPTS_DIR / "quality" / "run_quality_checks.py").read_text()
        assert "operating_cash_flow" in s and "capital_expenditure" in s and "0.005" in s
...
class TestDAG:
    DAG_FILE = "fundamentals_cash_flow_pipeline.py"
