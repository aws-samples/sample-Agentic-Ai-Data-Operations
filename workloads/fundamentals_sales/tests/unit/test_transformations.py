"""Unit tests for fundamentals_sales."""
...
class TestSourceConfig:
    def test_dedup_key(self):
        ...
        assert config["ingestion"]["dedup_key"] == ["entity_id", "fiscal_period_id", "metric"]
...
class TestSemanticConfig:
    ...
    def test_metric_is_dimension(self):
        with open(CONFIG_DIR / "semantic.yaml") as f:
            config = yaml.safe_load(f)
        metric_col = next(c for c in config["columns"] if c["name"] == "metric")
        assert metric_col["role"] == "dimension"

    def test_keeps_both_native_and_usd(self):
        with open(CONFIG_DIR / "semantic.yaml") as f:
            config = yaml.safe_load(f)
...
class TestTransformationConfig:
    def test_dedup_keep_latest(self):
        ...
        assert dedup["key"] == ["entity_id", "fiscal_period_id", "metric"]
...
class TestQualityConfig:
    ...
    def test_pk_uniqueness_critical(self):
        ...
        assert u[0]["columns"] == ["entity_id", "fiscal_period_id", "metric"]
        assert u[0]["severity"] == "critical"

    def test_fiscal_period_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        rule = next(r for r in validity if r.get("column") == "fiscal_period")
        assert rule["severity"] == "critical"

    def test_value_usd_nonnegative_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        rule = next(r for r in validity if r.get("column") == "value_usd")
        assert rule["severity"] == "critical"
...
class TestScripts:
    def test_silver_filters_fiscal_period(self):
        s = (SCRIPTS_DIR / "transform" / "bronze_to_silver.py").read_text()
        for v in ("FY", "Q1", "Q2", "Q3", "Q4"):
            assert f'"{v}"' in s

    def test_silver_dedups_on_pk(self):
        s = (SCRIPTS_DIR / "transform" / "bronze_to_silver.py").read_text()
        assert 'partitionBy("entity_id", "fiscal_period_id", "metric")' in s
...
class TestDAG:
    DAG_FILE = "fundamentals_sales_pipeline.py"
...
class TestSQL:
    ...
    def test_silver_pk_columns_not_null(self):
        ...
        for col in ("entity_id", "fiscal_period_id", "fiscal_year",
                    "fiscal_period", "metric", "currency", "value_usd"):
            line = next(line for line in sql.splitlines() if line.strip().startswith(col + " "))
            assert "NOT NULL" in line

    def test_silver_partitions_include_metric(self):
        sql = (SQL_DIR / "silver" / "create_silver_table.sql").read_text()
        assert "PARTITIONED BY (metric" in sql
