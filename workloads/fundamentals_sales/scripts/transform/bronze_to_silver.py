"""Silver transformation: type-cast, validate, latest-wins dedup for sales fundamentals.
...
df = (
    df.withColumn("fiscal_year", F.col("fiscal_year").cast(IntegerType()))
      .withColumn("period_end_date", F.to_date("period_end_date", "yyyy-MM-dd"))
...
quarantine_mask = (
    ...
    | F.col("metric").isNull()
    | (F.col("metric") == "")
    | ~F.col("fiscal_period").isin("FY", "Q1", "Q2", "Q3", "Q4")
)
quarantined = df.filter(quarantine_mask)
clean = df.filter(~quarantine_mask)

window = Window.partitionBy("entity_id", "fiscal_period_id", "metric") \
...
if quarantine_count > 0:
    quarantined.write.mode("append").parquet(
        f"s3://{args['data_lake_bucket']}/quarantine/fundamentals_sales/"
    )
    log.warn("Records quarantined", count=quarantine_count, reason="null PK or invalid fiscal_period")
