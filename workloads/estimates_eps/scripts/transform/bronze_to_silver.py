"""Silver transformation: type-cast, validate, latest-wins dedup for EPS estimates.
...
df = (
    df.withColumn("estimate_date", F.to_date("estimate_date", "yyyy-MM-dd"))
      .withColumn("eps_mean", F.col("eps_mean").cast(DoubleType()))
      .withColumn("eps_high", F.col("eps_high").cast(DoubleType()))
      .withColumn("eps_low", F.col("eps_low").cast(DoubleType()))
...
quarantine_mask = (
    ...
    | ~F.col("period_type").isin("NTM", "FY1", "FY2")
)
quarantined = df.filter(quarantine_mask)
clean = df.filter(~quarantine_mask)

window = Window.partitionBy("entity_id", "estimate_date", "period_type") \
...
if quarantine_count > 0:
    quarantined.write.mode("append").parquet(
        f"s3://{args['data_lake_bucket']}/quarantine/estimates_eps/"
    )
    log.warn("Records quarantined", count=quarantine_count, reason="null PK or invalid period_type")
