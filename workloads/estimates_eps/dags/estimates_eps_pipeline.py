"""Estimates EPS Pipeline DAG (manual trigger).
...
Pipeline (manual trigger): S3 Sensor -> Bronze Ingest -> Silver Transform -> Quality Gate.
"""
...
with DAG(
    ...
    description="Manual ETL: Bronze -> Silver for sell-side EPS estimates",
    schedule=None,
    ...
    tags=[WORKLOAD, "estimates", "manual"],
    doc_md=__doc__,
) as dag:
    ...
    wait_for_source = S3KeySensor(
        ...
        bucket_key=f"raw/estimates/eps/*.csv",
        poke_interval=300,
        timeout=3600,
        mode="poke",
    )
    ...
    with TaskGroup("bronze") as bronze_group:
        ingest = GlueJobOperator(
            ...
            script_args={
                ...
                "--source_prefix": "raw/estimates/eps/",
