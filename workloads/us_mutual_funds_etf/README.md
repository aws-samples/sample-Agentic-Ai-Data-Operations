...
- **Seed questions:** 6 example queries (documentation only — runtime NL→SQL lives in AWS Semantic Layer)
...
- Enable NLP queries via AWS Semantic Layer (when deployed) using the staged `ontology.ttl` + `mappings.ttl`
