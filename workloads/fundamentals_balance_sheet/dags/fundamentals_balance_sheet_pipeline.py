"""Fundamentals Balance Sheet Pipeline DAG (manual trigger)."""
...
with DAG(
    ...
    tags=[WORKLOAD, "fundamentals", "manual"],
    doc_md=__doc__,
) as dag:
    ...
    wait_for_source = S3KeySensor(
        ...
        bucket_key=f"raw/fundamentals/balance_sheet/*.csv",
        poke_interval=300,
        timeout=3600,
        mode="poke",
    )
    ...
    with TaskGroup("bronze") as bronze_group:
        ingest = GlueJobOperator(
            ...
            script_args={
                ...
                "--source_prefix": "raw/fundamentals/balance_sheet/",
