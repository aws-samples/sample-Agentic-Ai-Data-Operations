"""Unit tests for fundamentals_balance_sheet configs, scripts, DAG, and SQL."""
...
class TestTransformationConfig:
    def test_dedup_keep_latest(self):
        ...
        assert dedup["key"] == ["entity_id", "fiscal_period_id"]
...
class TestQualityConfig:
    ...
    def test_total_assets_positive_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        rule = next(r for r in validity if r.get("column") == "total_assets")
        assert rule["severity"] == "critical"
        assert "> 0" in rule["rule"]

    def test_value_usd_positive_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        validity = config["dimensions"]["validity"]
        rule = next(r for r in validity if r.get("column") == "value_usd_total_assets")
        assert rule["severity"] == "critical"

    def test_accounting_identity_critical(self):
        with open(CONFIG_DIR / "quality_rules.yaml") as f:
            config = yaml.safe_load(f)
        consistency = config["dimensions"]["consistency"]
        rule = next(
            r for r in consistency
            if "total_liabilities" in r.get("rule", "") and "total_equity" in r.get("rule", "")
        )
        assert rule["severity"] == "critical"
        assert "0.005" in rule["rule"]
...
class TestScripts:
    ...
    def test_quality_enforces_accounting_identity(self):
        s = (SCRIPTS_DIR / "quality" / "run_quality_checks.py").read_text()
        assert "total_liabilities" in s and "total_equity" in s and "0.005" in s
...
class TestDAG:
    DAG_FILE = "fundamentals_balance_sheet_pipeline.py"
...
class TestSQL:
    ...
    def test_silver_pk_columns_not_null(self):
        ...
        for col in ("entity_id", "fiscal_period_id", "fiscal_year",
                    "total_assets", "currency", "value_usd_total_assets"):
