"""Quality checks for fundamentals_balance_sheet Silver.
...
Critical:
  * Completeness on entity_id, fiscal_period_id, fiscal_year, total_assets,
    value_usd_total_assets, currency
  * Uniqueness of (entity_id, fiscal_period_id)
  * total_assets > 0 and value_usd_total_assets > 0
  * Accounting identity: total_assets ≈ total_liabilities + total_equity (0.5% tolerance)
...
log = StructuredLogger(
    agent="Quality Agent",
    workload="fundamentals_balance_sheet",
    run_id=args["run_id"],
)
...
for col in ("entity_id", "fiscal_period_id", "fiscal_year", "total_assets",
            "value_usd_total_assets", "currency"):
    checks_total += 1
...
checks_total += 1
nonpositive_assets = df.filter(F.col("total_assets") <= 0).count()
if nonpositive_assets == 0:
    checks_passed += 1
else:
    critical_failures.append(f"total_assets <= 0: {nonpositive_assets} row(s)")

checks_total += 1
nonpositive_usd = df.filter(F.col("value_usd_total_assets") <= 0).count()
if nonpositive_usd == 0:
    checks_passed += 1
else:
    critical_failures.append(
        f"value_usd_total_assets <= 0: {nonpositive_usd} row(s)"
    )

# Accounting identity within 0.5% tolerance.
checks_total += 1
identity_violation = df.filter(
    ...
    & (
        F.abs(
            F.col("total_assets")
            - (F.coalesce(F.col("total_liabilities"), F.lit(0))
               + F.coalesce(F.col("total_equity"), F.lit(0)))
        ) / F.col("total_assets")
        > 0.005
    )
).count()
if identity_violation == 0:
    ...
    critical_failures.append(
        f"accounting identity violated (>0.5% tolerance): {identity_violation} row(s)"
    )
