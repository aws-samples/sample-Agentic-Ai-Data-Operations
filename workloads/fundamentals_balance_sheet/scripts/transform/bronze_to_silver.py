"""Silver transformation: type-cast, validate, latest-wins dedup for balance sheet.
...
df = (
    df.withColumn("fiscal_year", F.col("fiscal_year").cast(IntegerType()))
      .withColumn("total_assets", F.col("total_assets").cast(DoubleType()))
      .withColumn("total_liabilities", F.col("total_liabilities").cast(DoubleType()))
      .withColumn("total_equity", F.col("total_equity").cast(DoubleType()))
      .withColumn("cash_and_equivalents", F.col("cash_and_equivalents").cast(DoubleType()))
      .withColumn("total_debt", F.col("total_debt").cast(DoubleType()))
      .withColumn("value_usd_total_assets", F.col("value_usd_total_assets").cast(DoubleType()))
...
if quarantine_count > 0:
    quarantined.write.mode("append").parquet(
        f"s3://{args['data_lake_bucket']}/quarantine/fundamentals_balance_sheet/"
    )
