...
When the user mentions Bronze/Silver/Gold work, identify the zone from their prompt and ask TARGETED questions for that zone.

> **Runtime note (Amazon Q Developer CLI):** there is no structured multi-choice UI
> primitive (the former `AskUserQuestion` tool) on this runtime. Present every
> discovery question as a plain markdown list; the user answers in free text, which
> you must parse yourself. The intent is unchanged — the human still explicitly
> answers every question before any artifact is generated.
