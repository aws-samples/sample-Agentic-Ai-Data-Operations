...
runtime: Amazon Q Developer CLI (legacy .amazonq/ layout)
---

# AmazonQ.md — Agentic Data Onboarding System

Bronze → Silver → Gold data pipeline orchestration. Multi-agent architecture with AWS integration.

> **Ported from `CLAUDE.md` for Amazon Q Developer CLI.** Content is unchanged except
> for runtime-specific references: rule paths point at `.amazonq/rules/`, sub-agent
> spawning is via `q chat --agent <name> --no-interactive` subprocesses rather than
> an in-model `Agent` tool, and the discovery gate uses plain conversational Q&A
> rather than a structured multi-choice UI. See `ADOP-to-AmazonQ-Migration-Design.md`.
...
**Identify the zone first**, then ask zone-targeted questions (see `.amazonq/rules/00-zone-questions.md`):
...
> **Runtime note:** there is no structured multi-choice UI on Amazon Q Developer CLI.
> Present discovery questions as plain markdown lists; the human answers in free text,
> which you parse yourself. The requirement — explicit human answers before any
> artifact is generated — is unchanged.
...
See `.amazonq/rules/01-human-gate-examples.md` for correct/incorrect behavior examples.
...
2. A `preToolUse` hook blocks any `fs_write` to those directories without the
   renderer token (`ADOP_RENDERER_TOKEN`). On Amazon Q Developer CLI this is a
   per-agent hook; blocking is signaled by exit code 2 + a STDERR reason (no
   JSON-stdout decision mechanism). See migration design Section 5.2 — hook ports
   land in Phase 2.
...
Coding conventions, error handling, logging protocol, testing strategy, transformation rules, and glossary are in `.amazonq/rules/`. On Amazon Q Developer CLI, **every file under `.amazonq/rules/` loads on every turn** — there is no glob-conditional (path-scoped) loading. Four rules are scoped by convention only; apply each only when the current task touches the matching file type, and ignore it otherwise:

| Rule | Apply only when editing |
|---|---|
| `03-python-airflow.md` | Python — `workloads/**/*.py`, `shared/**/*.py`, `dags/**/*.py`, `scripts/**/*.py` |
| `04-sql.md` | SQL — `**/*.sql`, `workloads/**/sql/**` |
| `05-yaml.md` | YAML config — `**/*.yaml`, `**/*.yml`, `workloads/**/config/**` |
| `08-transformations.md` | Transformation logic — `workloads/**/{scripts,sql}/**`, `workloads/**/config/transformations.yaml` |

The other six rules (`00`, `01`, `02`, `06`, `07`, `09`) are always-on regardless of task.
